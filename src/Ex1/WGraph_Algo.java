...
public class WGraph_Algo  implements weighted_graph_algorithms{
    ...
    public weighted_graph copy() {
        WGraph_DS brandNewGraph = new WGraph_DS();
        Collection<node_info> colNodes =  myGraph.getV();

        for(Iterator<node_info> node = colNodes.iterator(); node.hasNext();)
        {
            brandNewGraph.addNode(node.next().getKey());
        }

        for(node_info node: colNodes)
        {
            ArrayList<node_info> listOfNodes = (ArrayList<node_info>) myGraph.getV(node.getKey());
            int srcKey = node.getKey();

            for(node_info n : listOfNodes)
            {
                int key = n.getKey();
                brandNewGraph.connect(srcKey, key, myGraph.getEdge(srcKey, key));
            }
        }
        return brandNewGraph;
    }
    ...
    public boolean save(String file) {
        try
        {
            ...
            String data = myGraph.forSaving();
            writer.write(data);
            writer.close();
            System.out.println("saving file succeeded");
        }
        catch (Exception e)
        {
            e.printStackTrace();
            ...
            while(reader.hasNextLine())
            {
                ...
                if(!data.isEmpty())
                {
                    ...
                    while(!data.isEmpty())
                    {
                        ...
                        brandNewGraph.addNode(ni);
                        brandNewGraph.connect(node, ni, weight);
                        data = data.substring(end+1);
                    }
                }
            }
            myGraph = brandNewGraph;
            System.out.println("loaded file succeeded");
        }
        catch(Exception e)
        {
            e.printStackTrace();
